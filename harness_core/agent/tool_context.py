...
class ToolContext:
    ...
    def __repr__(self) -> str:
        ...
        if name is None and self.agent is not None:
            name = getattr(getattr(self.agent, "_agent_type", None), "name", None)
        return f"ToolContext(agent={name!r})"
...
def current_tool_context() -> "ToolContext":
    ...
    """
    return ToolContext(agent=CURRENT_AGENT.get())
