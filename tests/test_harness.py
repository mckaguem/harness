...
class TestRunLoop:
    ...
    def _setup_event_bridge(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda agent, content, resp, ctx_len, reasoning=None:
                terminal_io.display_agent_response(content, resp, ctx_len, reasoning=reasoning),
        )
        monkeypatch.setattr(
            loop_mod, "_emit_tool_call_event",
            lambda agent, func_name, args_str, summary=None, pre_content="", reasoning=None:
                terminal_io.display_tool_call(func_name, args_str, summary=summary,
        ...
        monkeypatch.setattr(
            ...
            lambda agent, func_name, result_title=None, result_display_text="",
                ...
                terminal_io.display_tool_result(func_name, result_title=result_title,
        ...
        monkeypatch.setattr(
            ...
            lambda agent, description:
                terminal_io.display_error(description or ""),
        )

    def test_run_loop_calls_print_system_on_start(self, monkeypatch):
        ...
        # Set up event bridge so emit_*_event helpers call display functions directly
        self._setup_event_bridge(monkeypatch)

        with patch("harness_core.terminal_io.display.print_system") as mock_print:
            ...
            with patch("harness_core.agent.loop.prompt_user", side_effect=side_effect):
                with patch("harness_core.terminal_io.display_agent_response") as mock_display:
    ...
    def test_run_loop_handles_exit_command(self):
        ...
        with patch("harness_core.terminal_io.print_system"):
    ...
    def test_run_loop_displays_tool_calls(self, monkeypatch):
        ...
        with patch("harness_core.terminal_io.print_system"):
            ...
            with patch("harness_core.agent.loop.prompt_user", side_effect=side_effect):
                with patch("harness_core.terminal_io.display_tool_call") as mock_display:
                    user_loop(mock_agent, mock_client)
    ...
    def test_run_loop_displays_tool_results(self, monkeypatch):
        ...
        with patch("harness_core.terminal_io.print_system"):
            ...
            with patch("harness_core.agent.loop.prompt_user", side_effect=side_effect):
                with patch("harness_core.terminal_io.display_tool_result") as mock_display:
                    user_loop(mock_agent, mock_client)
    ...
    def test_run_loop_displays_errors(self, monkeypatch):
        ...
        with patch("harness_core.terminal_io.print_system"):
            ...
            with patch("harness_core.agent.loop.prompt_user", side_effect=side_effect):
                with patch("harness_core.terminal_io.display_error") as mock_display:
                    user_loop(mock_agent, mock_client)
    ...
    def test_run_loop_handles_multiple_outputs(self, monkeypatch):
        ...
        with patch("harness_core.terminal_io.print_system"):
            ...
            with patch("harness_core.agent.loop.prompt_user", side_effect=side_effect):
                with patch("harness_core.terminal_io.display_agent_response") as mock_resp:
                    with patch("harness_core.terminal_io.display_tool_call") as mock_tool:
                        with patch("harness_core.terminal_io.display_tool_result") as mock_result:
                            user_loop(mock_agent, mock_client)
