...
## TUI message-window copy/paste — FIXED (SelectableStatic added + get_selection override)
- Symptom: selecting text in the message window (#output VerticalScroll) and copying did nothing; worked in the TextArea input. Root cause was TWO bugs:
  1. `SelectableStatic` was imported in `harness_tui.py` (lines 14, 105, 153, 195) from `harness_core.terminal_io.widgets` but was NEVER DEFINED anywhere → hard `ImportError` that broke the whole `terminal_io` package at import time. (Introduced in commit 60d618c which added the import but not the class.)
  2. Even after defining it, Textual's default `Static.get_selection` (`textual/widget.py:4213`) returns `None` for any non-`Text`/`Content` visual. The message window mounts `SelectableStatic(Panel(...))` where the Panel wraps `Markdown`/`Syntax`/`Text` — all become `RichVisual` (`textual/visual.py:110`), so copy yielded nothing. (Mouse *highlighting* worked because `Static.ALLOW_SELECT=True` and the screen-level selection gate doesn't require `can_focus`.)
- Fix: added `class SelectableStatic(Static)` to `harness_core/terminal_io/widgets.py` with `can_focus = True` and an overridden `get_selection(self, selection)` that, for a `RichVisual`, renders plain text via `visual.render_strips(self.size.width, self.size.height, self.visual_style, None)` and joins strip.text with "\n" (returns `(selection.extract(text), "\n")`). This makes Ctrl+C copy work on previous messages (Panel box-drawing included; Markdown/Syntax styling stripped to plain text — correct/expected).
- `TextArea` input was never affected (separate widget with its own selection). No CSS changes needed. Verified: `import harness_core.terminal_io` → IMPORT OK; `get_selection` returns non-None text for Panel-wrapped Markdown and Panel-wrapped Text inside `App.run_test()`.
- LESSON: when adding a `Static` subclass that wraps rich renderables (Panel/Markdown/Group/Syntax) and you want copy to work, you MUST override `get_selection` to extract from `RichVisual.render_strips` — `can_focus=True` alone does NOT enable copy. Also: never leave an imported-but-undefined class (it breaks the whole package import).

## CopyablePanel message widget (terminal_io)
- Every message panel in the TUI output pane is now a `CopyablePanel(Static)` (defined in `harness_core/terminal_io/widgets.py`), composed as a `Horizontal` of an inner `SelectableStatic(id="content")` + a `Button(id="copy-btn", "📋")`. `CopyablePanel` has `can_focus=True` and `BINDINGS=[("c","copy_text","Copy Panel")]`; `trigger_copy()` extracts plain text via `SelectableStatic.get_content_text()` and calls `app.copy_to_clipboard(text)` + `notify(...)`.
- `harness_tui.py` mounts `CopyablePanel` at all message sites: `write()`, `begin_tool_panel()` (inner = `CopyablePanel(id="tool-content")`), and the standalone branch of `complete_tool_panel()`. The merged tool-result path in `complete_tool_panel` updates the INNER `#content` `SelectableStatic` (`collapsible_inner.query_one("#content", SelectableStatic).update(merged)`), NOT the container's own `.update()` — because a Textual `Static` with children paints the children, not its own renderable.
- `SelectableStatic` keeps `can_focus=False` (so focus stays on the CopyablePanel parent) and its `get_selection`/`get_content_text` overrides (extract plain text from `RichVisual.render_strips` so copy works for Panel/Markdown/Group/Syntax).
- CRITICAL CSS CONSTRAINT: Textual does NOT support `position: absolute`, `right:`, or `top:` CSS properties. An earlier version of `CopyablePanel.DEFAULT_CSS` used `position: absolute; right:1; top:0;` on the button → `StylesheetParseError` → panel children never mounted → `NoMatches` on copy. The valid approach is a `Horizontal` layout (content `width:1fr` + button `width:4`). Never reintroduce absolute positioning for the copy button.
